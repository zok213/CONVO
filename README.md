...
No external API keys are required for the default pipeline -- models are billed through the Agora Console reseller agreement. See `app/api/invite-agent/route.ts` for BYOK (Bring Your Own Key) blocks that can be uncommented to use your own vendor credentials.
...
- `translator-app/.env.local` -- App credentials (gitignored, never commit)
- `translator-app/env.local.example` -- Template with all documented variables
